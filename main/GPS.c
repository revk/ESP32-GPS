...
#define	settings_mobile	\
	u16(mtu,1488,		UDP MTU)	\
      	bl(atdebug,N,		Modem debug log)    \
        io(atuart,2,		Modem UART ID)	\
        u32(atbaud,115200,	Modem Baud)	\
        io(attx,,		Modem Tx GPIO)	\
        io(atrx,,		Modem Rx GPIO)	\
        io(atkey,,		Modem power key GPIO)	\
        io(atrst,,		Modem reset GPIO)	\
...
fixstatus (void)
{
   ...
   revk_info (TAG, "Sats %d (NAVSTAR %d/%d, GLONASS %d/%d, GALILEO %d/%d) %s %s hepe=%.1f vepe=%.1f", sats, gngsa[0], gxgsv[0],
              gngsa[1], gxgsv[1], gngsa[2], gxgsv[2],
              fixtype == 0 ? "Invalid" : fixtype == 1 ? "GNSS" : fixtype == 2 ? "DGPS" : fixtype == 6 ? "Estimated" : "?",
   ...
   while (1)
   {
      ...
      while (1)
      {
         ...
         at_cmd (m95 ? "AT+QIHEAD=1" : "AT+CIPHEAD=1", 0, 0);   // Send IPD headers
         {
            try = 10;
            char temp[200];
            snprintf (temp, sizeof (temp), "AT+%s=\"UDP\",\"%s\",%ld", m95 ? "QIOPEN" : "CIPSTART", loghost, logport);
...
#if 0
         if (fixdebug)
            revk_info (TAG, "Logging %u/%u from %u fixes at %u." MPART "/%u." MPART "/%u." MPART " covering %u seconds", last, max,
                       fixsave, fix[1].dist / MSCALE, fix[1].dist % MSCALE, dkept / MSCALE, dkept % MSCALE, dlost / MSCALE,
